...
(defcustom standard-keys-C-x-prefix-for-descriptions ?\C-e
  "The remaped `C-x' key prefix to use in commands docstring descriptions.
This only have effect if `standard-keys-update-commands-descriptions' is
non-nil."
  :type 'key)

(defcustom standard-keys-C-c-prefix-for-descriptions ?\C-d
  "The remaped `C-c' key prefix to use in commands docstring descriptions.
This only have effect if `standard-keys-update-commands-descriptions' is
non-nil."
  :type 'key)
...
WARNING: This only should be used only for override properly the `C-x'
and `C-c' in buffers or modes which take precedence over
`standard-keys-mode' keymaps."
  :type 'boolean
  :risky t)
...
The value can be any of the following symbols:
...
(defun sk--get-key-in-active-mode-keymaps (key &optional maps)
  "Return a composed keymap from MAPS which defines KEY.
If MAPS is not set, it will use `current-active-maps' instead."
  (make-composed-keymap
   ;; The keymaps order must be inverted,
   ;; cl-loop already does this job.
   (cl-loop for maps in (or maps (current-active-maps))
            for k = (keymap-lookup (cdr maps) key)
            when k collect k)))
...
(defun sk--where-is-internal-advice (orig-fun def &optional keymap &rest rest)
  ...
  (unless keymap
    (let ((actives (current-active-maps))
          (C-x standard-keys-C-x-prefix-for-descriptions)
          (C-c standard-keys-C-c-prefix-for-descriptions))
      (setq keymap
            `(,@actives
              ,(make-composed-keymap
                `((,C-x keymap ,@(sk--get-key-in-active-mode-keymaps "C-x" actives))
                  (,C-c keymap ,@(sk--get-key-in-active-mode-keymaps "C-c" actives))))))))
...
(defvar-keymap standard-keys-default-keymap
  ...
  "C-e" (standard-keys-key-keybinding "C-x")
  "C-d" (standard-keys-key-keybinding "C-c")
...
This keymap is intended to be a minimal CUA, binding only a few keybindings,
it rebinds `C-x' and `C-c' to `s-x' and `s-c' respectively (the `s' is
for Super key, also know as Windows or Ubuntu key).
...
or with `use-package':
  ...
  ;; Move C-x and C-c to the Super key (aka. Windows/Ubuntu key)
  "s-x" (standard-keys-key-keybinding "C-x")
  "s-c" (standard-keys-key-keybinding "C-c")
...
(defvar-keymap standard-keys-ergoemacs-like-keymap
  :doc "`Ergoemacs US layout Standard'-like keymap for `standard-keys-map-style'.
...
or with `use-package':
  ...
  "<menu> d" (standard-keys-key-keybinding "C-x")
  "<menu> f" (standard-keys-key-keybinding "C-c")
